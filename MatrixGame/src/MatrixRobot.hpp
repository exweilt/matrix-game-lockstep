...
public:
    ...
    bool GetWeaponPos(D3DXVECTOR3 &pos) {
        if (m_Weapon) {
            pos = m_Weapon->GetPos();
            return true;
        }
    ...
    void SetArcadeCoefficient() {
        if (m_Weapon)
            m_Weapon->SetArcadeCoefficient();
    }
    void SetDefaultCoefficient() {
        if (m_Weapon)
            m_Weapon->SetDefaultCoefficient();
    }
...
public:
    ...
    CMatrixMapStatic *GetStatic(void) {
        if (m_ObjCore) {
            if (m_ObjCore->m_Object)
                return m_ObjCore->m_Object;
            m_ObjCore->Release();
            m_ObjCore = NULL;
        }
        return NULL;
    ...
    void Reset(void) { memset(this, 0, sizeof(SOrder)); }
...
public:
    ...
    static bool SphereToAABBCheck(const D3DXVECTOR2 &sphere_p, const D3DXVECTOR2 &vMin, const D3DXVECTOR2 &vMax,
                                  float &d, float &dx, float &dy);
    D3DXVECTOR3 SphereToAABB(const D3DXVECTOR2 &pos, const SMatrixMapMove *smm, const CPoint &cell,
    ...
    void ZoneMoveCalc(void);  // Рассчитать путь движения до ближайшей зоны (In: m_ZoneCur,m_ZoneNear) (Out:
                              // m_MovePathCnt,m_MovePathCur,m_MovePath)
    ...
    bool Seek(const D3DXVECTOR3 &dest, bool &rotate, bool end_path,
              bool back = false);                   // поиск вектора смещения робота
    ...
    void LowLevelMove(int ms, const D3DXVECTOR3 &dest, bool robot_coll, bool obst_coll, bool end_path = true,
                      bool back = false);
...
inline bool CMatrixRobotAI::PLIsInPlace(void) const {
    ...
    if (FindOrderLikeThat(ROT_MOVE_TO))
        return false;
    if (FindOrderLikeThat(ROT_MOVE_RETURN))
        return false;

    ...
    [[maybe_unused]] CMatrixMapStatic *attaker)
{
    ...
    if (m_Side == g_Network.controllable_side_id && !friendly_fire) {
...
void CMatrixBuilding::Takt(int cms) {
    ...
    if (m_Side == g_Network.controllable_side_id && FLAG(m_ObjectState, BUILDING_NEW_INCOME)) {
...
void CBuildStack::TickTimer(int ms) {
    ...
    if (m_Items && m_Top->IsRobot()) {
        ...
        if (m_Timer >= g_Config.m_Timings[UNIT_ROBOT] && m_ParentBase->m_State == BASE_CLOSED) {
            if (m_Top->GetSide() == g_Network.controllable_side_id && !FLAG(g_MatrixMap->m_Flags, MMFLAG_AUTOMATIC_MODE)) {
            ...
            // STUB:
            if (m_ParentBase->GetSide() == g_Network.controllable_side_id) {
        ...
        if (m_Timer >= g_Config.m_Timings[UNIT_TURRET]) {
            if (m_Top->GetSide() == g_Network.controllable_side_id) {
...
void CBuildStack::ClearStack() {
    while (DeleteItem(1)) {}
    if (m_ParentBase->m_Side == g_Network.controllable_side_id) {
